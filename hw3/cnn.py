...
unlabelin = np.zeros((45000, 3, 32, 32))
for i in xrange(45000):
	unlabelin[i, :, :, :] = all_unlabel[i].reshape((3,32,32))
...
if '-train' in sys.argv:
	...
	### Self-training ###
	if '-self' in sys.argv:
		sampleW = np.ones(5000)*10
		for it in xrange(4):
			if len(unlabelin) == 0:
				break
			unlabelout = model.predict(unlabelin)
			addList = np.amax(unlabelout, axis=1)>0.9
			addList = np.where(addList == True)
			addList = addList[0]
			print
			print '# of added unlabel data:', len(addList)
			print
			cv_train = np.concatenate((cv_train, unlabelin[addList, :, :, :]))
			cv_out = np.concatenate((cv_out, (unlabelout[addList, :]>0.9).astype(int)))
			unlabelin = np.delete(unlabelin, addList, axis=0)
			unlabelout = np.delete(unlabelout, addList, axis=0)
			
			sampleW = np.concatenate((sampleW, np.ones(len(addList))))
